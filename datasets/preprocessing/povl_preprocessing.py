...
class PovlPreprocessing(BasePreprocessing):
    def __init__(
        self,
        data_dir: str = "~/data/upcycling/povl_office",
        save_dir: str = "./data/processed/inference_test/",
        modes: tuple = (
            # "Area_1",
            # "Area_2",
            # "Area_3",
        ...
        self.inherit_label_database(path="./data/processed/s3dis/label_database.yaml")
        ...
        print('here')
    def inherit_label_database(self, path):
        """
        Copy label database from another dataset
        """
        import yaml

        with open(path, "r") as f:
            label_database = yaml.safe_load(f)
    ...
    def process_file(self, filepath, mode):
        ...
        for instance in [
            f
            for f in os.scandir(
                self.data_dir / mode / scene_name 
            )
            if f.name.endswith(".pts")
        ]:
            points = np.loadtxt(instance.path, skiprows=1)

            instance_points = points[:, [0,1,2,4,5,6]]
            instance_normals = np.ones((instance_points.shape[0], 3))
            instance_class = np.expand_dims(points[:,3],axis=1)
            instance_id = np.zeros((points.shape[0],1))
    ...
    def compute_color_mean_std(self, train_database_path: str = ""):
        area_database_paths = [
            ...
            if f.name.startswith("Area_") and f.name.endswith(".yaml")  # and 'mean_std' not in f.name
        ]
        ...
        for database_path in area_database_paths:
            ...
            if len(all_mean) == 0:
                # copy the only database to the new file
